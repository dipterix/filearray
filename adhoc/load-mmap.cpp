...
inline void subset_partition(
    ...
    for(; ii_idx2 < idx2len; ii_idx2++ , idx2ptr++) {
        ...
        //     (idx1_start + block_size * block);
        end_idx = start_idx - idx1_start + idx1_end + 1;
        
        if( start_idx >= mmap_size ){
            if( idx2_sorted ){
                break;
            }
            continue;
        }
        if( end_idx >= mmap_size ){
            end_idx = mmap_size;
        }
        
        mmap_ptr2 = mmap_ptr + start_idx;
        current_pos = start_idx;
        
        // while(current_pos < start_idx)
        // buffer_nelems
        transform(mmap_ptr2, mem_buffer, idx1_end - idx1_start + 1);
...
#pragma omp for schedule(static, 1) nowait
    for(R_xlen_t ii = 0; ii < niter; ii++){
        ...
        try {
            ...
            int64_t region_len = file_elem_size * (idx1_end - idx1_start + block_size * (idx2_end - idx2_start));
            boost::interprocess::mapped_region region(
                    fm, mode, 
                    FARR_HEADER_LENGTH + file_elem_size * (block_size * idx2_start + idx1_start), 
                    region_len);
            B* begin = static_cast<B*>(region.get_address());
            
            subset_partition(
                begin, region.get_size() / file_elem_size, 
